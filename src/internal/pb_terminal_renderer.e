...
feature -- Multiline conversion
	...
	redraw_line_sequence (a_line: READABLE_STRING_GENERAL; a_previous_count, a_rows_below: INTEGER): STRING_32
		...
		require
			...
			rows_below_non_negative: a_rows_below >= 0
		do
			if a_rows_below = 0 then
				Result := redraw_sequence (a_line, a_previous_count)
			else
				create Result.make (a_line.count + 32)
				Result.append_character ('%R')
				append_cursor_up (Result, a_rows_below)
				Result.append_string_general ({STRING_32} "%/27/[2K%R")
				Result.append_string_general (a_line)
				append_cursor_down (Result, a_rows_below)
				Result.append_character ('%R')
			end
		end
	...
	frame_sequence (a_previous_line_count: INTEGER; a_lines: ARRAYED_LIST [STRING_32]): STRING_32
		...
		require
			previous_count_non_negative: a_previous_line_count >= 0
		do
			create Result.make (a_previous_line_count * 8 + frame_character_count (a_lines) + 16)
			append_clear_frame (Result, a_previous_line_count)
			if a_previous_line_count = 0 and then not a_lines.is_empty then
				Result.append_character ('%R')
			end
	...
	commit_frame_sequence (a_previous_line_count: INTEGER; a_lines: ARRAYED_LIST [STRING_32]): STRING_32
		...
		require
			previous_count_non_negative: a_previous_line_count >= 0
		do
			Result := frame_sequence (a_previous_line_count, a_lines)
	...
	message_frame_sequence (a_message: READABLE_STRING_GENERAL; a_previous_line_count: INTEGER; a_lines: ARRAYED_LIST [STRING_32]): STRING_32
		...
		require
			previous_count_non_negative: a_previous_line_count >= 0
		do
			if a_previous_line_count = 1 and then a_lines.count = 1 then
				Result := message_sequence (a_message, a_lines.first, a_lines.first.count)
			else
				create Result.make (a_message.count + a_previous_line_count * 8 + frame_character_count (a_lines) + 16)
				append_clear_frame (Result, a_previous_line_count)
...
feature -- Conversion
	...
	message_sequence (a_message, a_line: READABLE_STRING_GENERAL; a_previous_count: INTEGER): STRING_32
		...
		local
			padding: STRING_32
		do
			create Result.make (a_message.count + a_line.count + a_previous_count + 4)
			Result.append_character ('%R')
			if a_previous_count > 0 then
				create padding.make_filled (' ', a_previous_count)
				Result.append (padding)
			end
			Result.append_character ('%R')
			Result.append_string_general (a_message)
...
feature {NONE} -- Multiline implementation
	...
	append_clear_frame (a_sequence: STRING_32; a_line_count: INTEGER)
		...
		require
			line_count_non_negative: a_line_count >= 0
		local
			index: INTEGER
		do
			from
	...
	frame_character_count (a_lines: ARRAYED_LIST [STRING_32]): INTEGER
		...
		do
			...
			as
				line_cursor
			loop
				Result := Result + line_cursor.count
			end
		end
