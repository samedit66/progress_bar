...
feature -- Test
	...
	test_basic_known
		...
		do
			...
			assert_true (
				"bar",
				attached last_text as text and then
					text.has_substring ("[###############---------------]")
			)
			assert_true (
				"percentage",
				attached last_text as text and then
					text.has_substring ("50%%")
			)
			assert_true (
				"counter",
				attached last_text as text and then
					text.has_substring ("5 / 10")
			)
			bar.finish
		end
	...
	test_standard_copies_affixes
		...
		do
			...
			unit := "classes"
			post_label := "ready"
			selected_formatter := formatters.standard (
				label,
				unit,
				post_label
			)
			...
			assert_true (
				"copied unit",
				attached last_text as text and then
					text.has_substring ("classes")
			)
			assert_true (
				"copied post label",
				attached last_text as text and then
					text.ends_with ("ready")
			)
			bar.finish
		end
	...
	test_unicode
		...
		do
			...
			assert_true (
				"full block",
				attached last_text as text and then
					text.has_code (0x2588)
			)
			assert_true (
				"empty block",
				attached last_text as text and then
					text.has_code (0x2591)
			)
			bar.finish
		end
