...
feature -- Test
	...
	test_redraw_longer_line
		...
		local
			sequence: STRING_32
		do
			sequence := redraw_sequence ("long", 2)
	...
	test_redraw_shorter_line
		...
		local
			sequence: STRING_32
		do
			sequence := redraw_sequence ("new", 5)
	...
	test_finish_frame
		...
		do
			create lines.make (2)
			sequence := finish_frame_sequence (0, 0, lines)
			assert_true ("empty idle frame", sequence.is_empty)
			sequence := finish_frame_sequence (1, 4, lines)
			...
			sequence := finish_frame_sequence (2, 4, lines)
	...
	test_message_above_active_line
		...
		local
			sequence: STRING_32
		do
			sequence := message_sequence ("done", "work", 4)
	...
	test_unicode_multiline_message
		...
		local
			sequence: STRING_32
		do
			sequence := message_sequence ({STRING_32} "first%NВторая%N", {STRING_32} "Работа", 6)
	...
	test_redraw_parent_line
		...
		local
			sequence: STRING_32
		do
			sequence := redraw_line_sequence ("parent", 3, 1)
	...
	test_replace_multiline_frame
		...
		do
			...
			lines.extend ("parent")
			lines.extend ("child")
			sequence := frame_sequence (1, lines)
